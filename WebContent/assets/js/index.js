 ...
 * default_placeholder 寮€鍙戣€呰嚜瀹氫箟榛樿鍥剧墖寤惰繜鍔犺浇鏂瑰紡锛�'base64'锛氱伆鑹茶儗鏅紱鑷畾涔夎儗鏅浘鐗囪矾寰勶紱'blur'锛氬浘鐗囬珮鏂ā绯婏紱榛樿涓�'blur'
 * met_prevarrow,met_nextarrow slick鎻掍欢缈婚〉鎸夐挳鑷畾涔塰tml
 * device_type       瀹㈡埛绔垽鏂紙d锛歅C绔紝t锛氬钩鏉跨锛宮锛氭墜鏈虹锛�
 */
METUI_FUN['head_nav_met_27_1'] = {
    name:'head_nav_met_27_1',
    init: function() {
        var wh=$(window).height(),
        $metnav_fixed=$("body.met-navfixed .head_nav_met_27_1"),
        $metnav_lang=METUI['head_nav_met_27_1'].find('.met-langlist'),
        $metnav_member=METUI['head_nav_met_27_1'].find('.navbar-toggler'),
        $met_navlist=METUI['head_nav_met_27_1'].find('.navlist');
        // 澶撮儴鍥哄畾妯″紡
        ...
        // 澶撮儴闃村奖
            ...
            if(device_type!='m'){
                // 涓嬫媺鑿滃崟浜岀骇鏍忕洰杩囧鏃朵笁绾ф爮鐩睍绀哄湪浜岀骇鏍忕洰涓嬮潰
        ...
        // 澶勭悊瀵艰埅杩囬暱
        // if($('.two-menu').length && $(window)>768){
        //     $('.two-menu').each(function() {
    ...
    cntotc:function(){
            //绠€浣撶箒浣撲簰鎹�
            var b=METUI['head_nav_met_27_1'].find('.btn-cntotc');
    ...
    nav:function (){
        // 澶撮儴鏍囪
        var $metcolumn_nav=METUI['head_nav_met_27_1'].find('.head-list');
        if($metcolumn_nav.length){
            $metcolumn_nav.navtabSwiper();
        }
    }
};
var x = new metui(METUI_FUN['head_nav_met_27_1']);
...
METUI_FUN['notice_list_met_27_1'] = {
    name: 'notice_list_met_27_1',
    init: function() {
        var n = METUI['notice_list_met_27_1'].find('.notice-swiper');
    if(n.length>0){
    ...
    appear: function() {
        if (METUI['notice_list_met_27_1'].length) {
            var repeat = false;
            function animation() {
                if (repeat == false) {
                    var m = METUI['notice_list_met_27_1'];
                        a = TweenMax.to(m, 0.5, {
                            autoAlpha: 1,
                            y:0,
                            ease: Power3.easeOut,
            ...
            var scene = new ScrollMagic.Scene({
                triggerElement: '.notice_list_met_27_1',
                offset: -300
            }).addTo(controller);
...
var notice = new metui(METUI_FUN['notice_list_met_27_1']);

METUI_FUN['news_list_met_27_1'] = {
    name: 'news_list_met_27_1',
    init: function() {
    	var _imgnews=METUI['news_list_met_27_1'].find(".imgnews-slick");
        if(_imgnews.length>0){
            _imgnews.slick({
            dots: true,
            fade:true,
            autoplay:true,
...
var banner = metui(METUI_FUN['news_list_met_27_1']);



METUI_FUN['news_list_met_27_3'] = {
    name: 'news_list_met_27_3',
    init: function() {
	    $(".bqy").click(function(){
		   var a = $(this).attr("data-id");
		   $(this).addClass("active").parent().siblings().children().removeClass("active");
		   for(var i=0;i<$(".productimg>div").length;i++){
		   		if(a==$(".productimg>div:eq("+i+")").attr("data-type")){
		   			$(".productimg>div:eq("+i+")").css("display","block").siblings().css("display","none");
		   		}
		   }
		});
    }
};
var banner = metui(METUI_FUN['news_list_met_27_3']);