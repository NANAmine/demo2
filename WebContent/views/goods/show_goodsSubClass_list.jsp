...
<div class="admin-content">
  <div class="am-cf am-padding">
    <div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">装备小类管理</strong> / <small>class</small></div>
  </div>

<!-- 查询条件部分开始 -->
<form id="data_form" class="am-form" action="show_goodsSubClass_list.do" method="post">
...
<!-- 查询条件部分结束 -->  
  ...
  <div class="am-g">
      <div class="am-u-sm-12">
          ...
          <thead>
              <tr >
                <th class="table-title "width="20%">编号</th>
                <th class="table-title" width="40%">大类</th>
                <th class="table-title" width="40%">小类</th>
              </tr>
          </thead>
          <c:forEach var="goodsSubClass" items="${goodsSubClass_list}" varStatus="s">
           	  <tr class="tr_click" data_id="${goodsSubClass.id}">
                <td width="20%">${goodsSubClass.id}</td>
                <td width="40%">${goodsSubClass.goodsClass.name}</td>
                <td width="40%">${goodsSubClass.name}</td>
              </tr>
          </c:forEach> 
        </table>
        <div class="am-cf">
...
$(function() {
	...
	$("#save_button").click(function() {
		...
		$.ajax({
		    ...
		    type : "POST",
		    url : "<%=request.getContextPath()%>/add_goodsSubClass.do?class_id="+$("#class_id").val(),
	 ...
	 $(".tr_click").click(function(){
		 $.ajax({
				contentType:"application/json",
			    type : "GET",
			    url : "<%=request.getContextPath()%>/get_goodsSubClass.do?id="+$(this).attr("data_id"),
			    dataType: "json",
			    success : function(data) {
			        if(data.flag=="1"){
			        	$("#name").val(data.goodsSubClass.name);
			        	$("#class_id").val(data.goodsSubClass.goodsClass.id);
			        	$("#id").val(data.goodsSubClass.id);
			        }
			        else{
			            alert("保存失败！");
			        }
	 ...
	 //删除按钮
	  $("#delete_button").click(function(){
		 $.ajax({
				contentType:"application/json",
			    type : "GET",
			    url : "<%=request.getContextPath()%>/delete_goodsSubClass.do?id="+$("#id").val()+"",
			    dataType: "json",
			    success : function(data) {
			        if(data.flag=="1"){
			        	$("#clear_button").click();
