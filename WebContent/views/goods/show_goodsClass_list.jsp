...
<div class="admin-content">
  <div class="am-cf am-padding">
    <div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">装备大类管理</strong> / <small>class</small></div>
  </div>

<!-- 查询条件部分开始 -->
<form id="data_form" class="am-form" action="show_goodsClass_list.do" method="post">
      ...
      <div class="am-u-sm-12">
          ...
          <thead>
              <tr >
                ...
                <th class="table-title" width="80%">大类</th>
              </tr>
          </thead>
          <c:forEach var="goodsClass" items="${goodsClass_list}" varStatus="s">
           	  <tr class="tr_click" data_id="${goodsClass.id}">
                <td width="20%">${goodsClass.id}</td>
                <td width="80%">${goodsClass.name}</td>
              </tr>
          </c:forEach> 
        </table>
        <div class="am-cf">
...
$(function() {
	...
	$("#save_button").click(function() {
		...
		$.ajax({
		    ...
		    type : "POST",
		    url : "<%=request.getContextPath()%>/add_goodsClass.do",
	 ...
	 $(".tr_click").click(function(){
		 $.ajax({
				contentType:"application/json",
			    type : "GET",
			    url : "<%=request.getContextPath()%>/get_goodsClass.do?id="+$(this).attr("data_id"),
			    dataType: "json",
			    success : function(data) {
			        if(data.flag=="1"){
			        	$("#name").val(data.goodsClass.name);
			        	$("#id").val(data.goodsClass.id);
			        }
			        else{
			            alert("保存失败！");
			        }
	 ...
	 //删除按钮
	  $("#delete_button").click(function(){
		 $.ajax({
				contentType:"application/json",
			    type : "GET",
			    url : "<%=request.getContextPath()%>/delete_goodsClass.do?id="+$("#id").val()+"",
			    dataType: "json",
			    success : function(data) {
			        if(data.flag=="1"){
